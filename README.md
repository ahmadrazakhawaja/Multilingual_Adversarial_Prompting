...
### How to run
1. Install Python on the system (tested with version 3.12.8). Using a different version may cause compatibility issues with some libraries.
2. (Recommended but not mandatory) setup python virtual envionment:
  - create a virtual envionment in the directory using `python -m venv venv`
  - activate virtual envionment through:
    - macos: `source venv/bin/activate`
    - windows: `venv\Scripts\activate`
3. install packages: run the command `pip install -r requirements.txt` to install the required packages for the project.
4. execute the project by first navigating to `src` folder: `cd src`, then run the project using the command: `python3 main.py`
